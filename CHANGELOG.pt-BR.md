...
Documentação bilíngue, com o inglês como versão apresentada no portfólio.

### Alterado

- `README.md`, `CHANGELOG.md` e `ENGINEERING_REVIEW.md` passam a ser as
  versões em inglês; os `*.pt-BR.md` guardam o português. Cada um traz um
  seletor de idioma na primeira linha.
- O README em inglês ganhou um **glossário de domínio** (CNPJ, CNAE,
  competência, Estabelecimentos/Empresas) e uma explicação curta das duas
  propriedades da fonte que moldam a arquitetura: snapshots mensais em vez de
  log de mudanças, e ~85 GB de dado sujo repartido em fatias. Sem esse
  contexto as decisões de projeto são ilegíveis para quem é de fora do Brasil.
- Os dois READMEs foram equalizados e atualizados com o que se aprendeu desde
  então: instabilidade das fatias entre competências, o portão de sanidade
  `diagnostico_eventos.py`, as duas regras de ancoragem temporal e a nota
  sobre testes de regressão que não conseguem falhar.
- `REVISAO_PD_V2.md` virou `ENGINEERING_REVIEW.md` (+ `.pt-BR`). O arquivo
  antigo é só um ponteiro e pode ser removido com `git rm`. As referências em
  `run_pipeline.py`, `scripts/carregar_icp.py` e `gld_icp_matches.sql` foram
  atualizadas.
- Na revisão, os itens 3.1, 3.3 e 3.6 estão marcados como **RESOLVIDO na
  2.2.0** — mantidos no registro, já que os achados explicam como os defeitos
  sobreviveram.
